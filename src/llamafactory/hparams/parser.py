...
from ..extras.compile_shims import apply_compile_shims
...
from .fsdp_validator import FsdpValidator
...
def _validate_fsdp_config() -> None:
    """Validate FSDP configuration if ACCELERATE_CONFIG_FILE is set."""
...
def _log_fsdp_runtime_status() -> None:
    ...
    try:
        ...
        logger.info_rank0(f"FSDP runtime status check failed: {e}")
...
def _verify_model_args(
    ...
    if model_args.enable_qat and model_args.quantization_bit is not None:
        raise ValueError(
            "QAT (Quantization Aware Training) is not compatible with existing quantization. Please disable quantization_bit."
        )
...
def _check_extra_dependencies(
    ...
    if training_args is not None:
        if training_args.deepspeed:
            ...
            check_version("deepspeed>=0.16.0,<=0.18.0", mandatory=True)
    ...
    try:
        ...
        if _compile_on and (_liger_on or _ds_on):
            ...
            try:
                ...
                dynamo_config.capture_scalar_outputs = True  # type: ignore[attr-defined]
            except Exception:
                pass
            # Reduce noisy Dynamo warnings in logs
            try:
